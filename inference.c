...
//回転
void rotate180 (int n, int m, float * x, float * o) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            o[j + i * n] = x[m * n - (j + i * n)];
        }
...
void rotate90 (int n, int m, float *x, float *o) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            o[j + i * n] = x[m * (n - j - 1) + i];
        }
...
//ノイズ関数
void makenoise (int n, int m, float *x, float *o) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            o[j + i * n] = x[j + i * n] + GetRandom(0, 1e-5);
        }
    }
}
...
void Adam(int m, 
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            m_b_hat[i] = m_b[i] / (1 - powf(rho1 , t) + eps);
            v_b_hat[i] = v_b[i] / (1 - powf(rho2 , t) + eps);
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                m_A_hat[i * n + j] = m_A[i * n + j] / (1 - powf(rho1 , t) + eps);
                v_A_hat[i * n + j] = v_A[i * n + j] / (1 - powf(rho2 , t) + eps);
...
// を使用することができる．
    ...
    float *x1 = malloc(sizeof(float) * 784);
    float *x2 = malloc(sizeof(float) * 784);
    //パラメタの初期化
        ...
        for (i = 0; i < num_epoch; i++) {
            ...
            for (j = 0; j < num_train; j++) {
                ...
                for (k = 0; k < batch_size; k++) { 
                    ...
                    rotate180(28, 28, train_x + 784 * index[100 * j + k], x1);
                    backward6(A1, b1, A3, b3, A5, b5, x1, train_y[index[100 * j + k]], y6, dA1, db1, dA3, db3, dA5, db5);
                    SGD(784, 50, dA1, dA1ave, db1, db1ave, batch_f, learning_rate, A1, b1);
                    SGD(50, 100, dA3, dA3ave, db3, db3ave, batch_f, learning_rate, A3, b3);
                    SGD(100, 10, dA5, dA5ave, db5, db5ave, batch_f, learning_rate, A5, b5);
                    rotate90(28, 28, train_x + 784 * index[100 * j + k], x2);
                    backward6(A1, b1, A3, b3, A5, b5, x2, train_y[index[100 * j + k]], y6, dA1, db1, dA3, db3, dA5, db5);
                    SGD(784, 50, dA1, dA1ave, db1, db1ave, batch_f, learning_rate, A1, b1);
                    SGD(50, 100, dA3, dA3ave, db3, db3ave, batch_f, learning_rate, A3, b3);
                    SGD(100, 10, dA5, dA5ave, db5, db5ave, batch_f, learning_rate, A5, b5);
                }
